...
/**
 * Checks if the given password is acceptable, and throws an error if not.
 *
 * @private
 * @param {string} str - The string.
 */
function checkAcceptablePassword(str) {
    ...
    if (str.length > BCRYPT_PW_MAXLEN) {
...
class Users {
    ...
    async insert(id, data) {
        ...
        for (let key of [
            ...
            if (!(key in data)) {
                throw new Error(`Missing "${key}" in request data.`);
            }
        }
        ...
        checkAcceptablePassword(password);
